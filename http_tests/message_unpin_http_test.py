...
def test_message_pin_more():
    ...
    requests.post(f"{url}/message/pin/v1", json = {
        'token': user2.json()['token'],
        'message_id': message_id4.json()['message_id']
    })

    requests.post(f"{url}/message/pin/v1", json = {
        'token': user2.json()['token'],
        'message_id': message_id5.json()['message_id']
    })
    ...
    assert payload['messages'][0]['is_pinned'] == False
    assert payload['messages'][1]['is_pinned'] == True
    assert payload['messages'][2]['is_pinned'] == True
    assert payload['messages'][3]['is_pinned'] == True
    assert payload['messages'][4]['is_pinned'] == False
    assert payload['messages'][5]['is_pinned'] == False
...
def test_message_pin_unauthorized_notmember():
    ...
    assert msg_id1_unpin.status_code == 403
